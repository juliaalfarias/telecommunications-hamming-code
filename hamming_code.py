...
original_msg = str(input(f"{Fore.GREEN}{msg}{Fore.RESET}\n"))
sigma = np.sqrt(float(input("Enter the value of variance σ^2: \n")))
...
# Add noise with zero mean and standard deviation 'sigma' to each component of the
...
for i in range(0, len(h_rec), 8):
    ...
    recovered_msg += chr(int(character))
...
for i in range(len(original_msg)):
    if recovered_msg[i] != original_msg[i]:
        errors += 1
error_rate = errors / len(original_msg)

duration = time.time() - start_time
print(f"Recovered message: {Fore.GREEN}{recovered_msg}{Fore.RESET}")
print(f"Message size: {Fore.YELLOW}{len(original_msg)}{Fore.RESET}")
print(f"Error rate: {Fore.RED}{100 * errors / len(recovered_msg): .2f}%{Fore.RESET}")
print(f"Total errors: {Fore.RED}{errors}{Fore.RESET}")
